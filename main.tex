...
\begin{lstlisting}
https://github.com/RavishaJain/Assigment_3/blob/main/codes/UTO.py
\end{lstlisting}
The CDF is plotted in Figure given below
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{./Figs/UTO_cdf}
...
Note that the function 
		\begin{align}
			v = f(u) = -2\ln{(1 - u)}
		\end{align}
is monotonically increasing in [0, 1] and $v \in \mathbb{R^+}$. Hence, it is invertible and that inverse function is given by
		\begin{align}
			u = f^{-1}(v) = 1 - \exp{\left(-\frac{v}{2}\right)}
		\end{align}
		Therefore, from the monotonicity of $v$, and using the cdf of uniform variables,
		\begin{align}
			F_U(x) = 
			\begin{cases}
				0 & x < 0 \\
